...
def build_prompt(text, keys, few_shot=True):
    key_str = ", ".join(keys)
    prefix = (
        "Extract the fields as compact JSON. "
        "Return only JSON with exactly the requested keys.\n"
    )
    if few_shot:
        prefix += (
            'Example:\nText: Name: Alice. Age: 24. City: Boston.\n'
            'Keys: name, age, city\nJSON: {"name":"Alice","age":24,"city":"Boston"}\n\n'
        )
...
def score_json_response(response, target):
    ...
    reward = (
        0.20 * details["valid_json"]
        + 0.10 * details["no_extra_text"]
        + 0.20 * details["keys"]
        + 0.15 * details["types"]
        + 0.25 * details["values"]
        + 0.10 * details["exact"]
...
def rollout_batch(learner_policy, actor, actor_fp16, tokenizer, actor_state, device, args):
    ...
    for _ in range(args.batch_size):
        ...
        reward, details = score_json_response(response, example.target)
...
def plot_metrics(logs, out_path, title):
    ...
    movement = [row["abs_logprob_movement"] for row in logs]
...
def evaluate_policy(policy, tokenizer, eval_examples, device, args):
    ...
    for example in eval_examples:
        ...
        reward, reward_detail = score_json_response(response, example.target)
        rewards.append(reward)
    ...
    for update in range(1, args.updates + 1):
        ...
        if update % args.log_every == 0 or update == 1:
            print(
                ...
                f"clip={row['clip_fraction']:.3f} "
                f"|move|={row['abs_logprob_movement']:.4f} "
                f"loss={row['loss']:.4f}"
            )
